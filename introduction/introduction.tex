...
This effect seems to be primarily driven by males, with crossover occurring more frequently in telomeric reions, while females have higher recombination rates closer to the centromeres\cite{Kong2002,Coopo2008,Kong2010}.
...
\citet{Berg2011} found that variation in PRDM9 alleles greatly influences hotspot usage in African populations, who appear to have a distinct subset of hotspots\cite{Hinch2012}.
...
Crossover interference, first observed in flies in 1913\cite{Sturtevant1913}, is another aspect that affects the placement on crossover events on a chromosome.
...
\citet{Kleckner2004} propose a model in which interference is governed by mechanical forces.
...
Therefore, extensions to this model were made to allow a second class of non-interfering crossovers to exist along-side those that are interfereing (see ``The two-pathway model'' on page \pageref{cointTPM}).
...
\paragraph{The simple gamma model.}
In the gamma model, all crossovers are assumed to be capable of interfering with each other.
The distribution inter-crossover distances are modeled to follow a gamma distribution with a single parameter, $\nu$, representing the strength of interference.
The shape and rate parameters of the gamma distribution are not independent, but instead represented with shape of $\nu$ and rate 2$\nu$.
The model assumes that chiasma in a tetrad are thinned to become crossovers.

The gamma distribution has a mean of $\frac{1}{2}$ Morgans, corresponding to a probability of a chiasma becoming a crossover is 0.5.
Therefore, the average distance between chiasma is $\frac{1}{2}$ Morgans, and the corresponding distance between crossovers is 1 Morgan.

(because X crossovers are expected per tetrad), and a standard deviation of $\frac{1}{2 \sqrt{\nu} }$.

\paragraph{The two pathway model.} \label{cointTPM}
